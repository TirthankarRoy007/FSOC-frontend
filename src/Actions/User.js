...
export const registerUser = (name, email, password, company, secretQuestion, answer) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("/register", { name, email, password, company, secretQuestion, answer }, {
            headers: {
                "Content-Type": "application/json",
            },
        })
...
export const forgotPassword = (email, secretQuestion) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("/forgot", { email, secretQuestion }, {
            headers: {
                "Content-Type": "application/json",
            },
        })
...
export const resetPasswordRequest = (email, secretQuestion, answer, newPassword) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post("/reset", { email, secretQuestion, answer, newPassword }, {
            headers: {
                "Content-Type": "application/json",
            },
        })
